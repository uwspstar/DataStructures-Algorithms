...
//终止只有两种情况，sum大于target和sum等于target。
{
    var combinationSum = function (nums, target) {
        ...
        const backTracking = (remain, start) => {
            console.log('backTracking (',remain,',',start,')');
            if (0 === remain) {
                console.log('path',path);
                res.push([...path]);
                return;
            } else if (remain < 0) { // < 0  is important !!!, it is finishing recursive, and move to next number
                return;
            } else {
                for (let i = start; i < nums.length; i++) {
                    path.push(nums[i]);
                    //console.log('start=', start,'nums[i]=',nums[i],'i=', i);
