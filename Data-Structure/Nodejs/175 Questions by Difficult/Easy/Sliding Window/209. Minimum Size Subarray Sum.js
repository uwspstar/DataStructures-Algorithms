...
Example 2: Input: target = 4, nums = [1,4,4] Output: 1 : [4]
    ...
    var minSubArrayLen = function (target, nums) {
        ...
        while (fast < nums.length) {
            if (maxSum < target) {
                ...
                maxSum -= nums[slow++]; // maxSum = maxSum - nums[slow]; slow++;
            }
        }
...
}
{
    ...
    //主要是看每一个元素被操作的次数，每个元素在滑动窗后进来操作一次，出去操作一次，每个元素都是被被操作两次，所以时间复杂度是2 * n 也就是$O(n)
